        ...
        <form id="clientForm" data-id="${editing ? c.id : ""}">
          <div class="form-grid">
            ...
            <div class="full ine-upload-preview" data-ine-preview>
              ${c.ineFoto ? `<img src="${esc(c.ineFoto)}" alt="INE actual">` : `<span>${icon("camera")} Sin foto de INE capturada</span>`}
            </div>
...
async function loadInePhoto(input) {
  ...
  try {
    ...
    if (preview) preview.innerHTML = `<img src="${esc(finalSrc)}" alt="Foto de INE">`;
    ...
    console.error(error);
    if (preview) preview.innerHTML = `<span>${icon("camera")} No se pudo procesar la foto</span>`;
